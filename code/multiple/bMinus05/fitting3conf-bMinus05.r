# fitting3conf-bMinus05.r - doing confidence intervals for b=-0.5.
#  25th April 2016.

# fitting3conf-bMinus25.r - doing confidence intervals for b=-2.5.
#  2nd December 2015.

# fitting3conf-bMinus15.r - doing confidence intervals for b=-1.5.
#  2nd December 2015.
...
postscript("fitting3conf-bMinus05.eps", height = figheight, width = figwidth,
...
LT.rep.conf.sort = confPlot(LT.rep.conf, legName="(b) LT", yLab="",
    xLim = c(-1, 0), yLabels=FALSE)

LTplus1.rep.conf.sort = confPlot(LTplus1.rep.conf, legName="(c) LTplus1")

xLimCom = c(-1, 0)      # common width of axes for the remainder, except LCD
...
LCD.rep.conf.sort = confPlot(LCD.rep.conf-1, legName="(g) LCD",
    xLim = c(-1.55, -0.35), vertFirst = TRUE,
    insetVal = c(-0.08+0.2, -0.06), insetVal2 = c(-0.08+0.2, 0.07))
...
postscript("fitting3confMLEfix-bMinus05.eps", height = 0.8*figheight,
           width = 0.8*figwidth,
           horizontal=FALSE,  paper="special")  
