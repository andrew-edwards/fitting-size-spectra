...
postscript("fitting2a-bMinus05.eps", height = figheight, width = figwidth,
...
axis(2, at = c(1.5, 2, 2.5), mgp=mgpVals)
axis(2, at = seq(0.5, 2.5, by=0.1), mgp=mgpVals, tcl=-0.2, labels=rep("", 21))
...
plot(hLBNbiom.list[["binVals"]]$log10binMid,
     ...
     ylim=c(1.3, max(hLBNbiom.list[["binVals"]]$log10totalBiom)), yaxt="n")
...
axis(2, at = 1:6, mgp=mgpVals)
axis(2, at = seq(1, 6, by=0.25), mgp=mgpVals, tcl=-0.2, labels=rep("", 21))
...
axis(2, at = seq(0, 5, by=0.25), mgp=mgpVals, tcl=-0.2, labels=rep("", 21))
lm.line(hLBNbiom.list[["binVals"]]$log10binMid, hLBNbiom.list[["norm.lm"]])
legJust(pos="bottomright", c("(f) LBNbiom", paste("slope=",
...
legJust(pos="bottomleft",
    c("(g) LCD", paste("slope=", signif(hLCD.slope, 3), sep=""),
...
legJust(pos="bottomleft",
        c("(h) MLE", paste("b=", signif(PLB.bMLE, 3), sep="")),
        logxy=TRUE, inset=c(-0.04, -0.04))
...
#  code.
postscript("standHist-bMinus05.eps", height = 2.7, width = figwidth/2,
           horizontal=FALSE,  paper="special")  #  height=4, width=4   was 7,4
par(mai=c(0.6, 0.6, 0.2, 0.3))
# mgpVals = c(2, 0.5, 0)            # mgp values   2.0, 0.5, 0
# For b=-0.5, distribution is so flat that do not really need gap in histogram,
#  but putting one in just to keep code the same. Could just plot a standard
#  histogram. Not going into Appendix, so haven't completely polished it up.
gap.barplot.cust(hLlin.list$counts, gap=c(150,200),
                 ytics = c(seq(0, 150, by=25), seq(200, 400, by=25)),
                                        #  = ...
                 midpoints = hLlin.list$mids,
                 breakpoints = hLlin.list$breaks,
                 xlim = c(-10,max(hLlin.list$breaks)+10),
                 yaxs="i",
                 ylim= c(0, 300),  # max = max(y) - gap[2] + gap[1] + some space
...
save.image(file = "fitting2-bMinus05.RData")
