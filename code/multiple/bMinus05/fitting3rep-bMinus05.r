# fitting3rep-bMinus05.r - redoing with b=-0.5. 25/4/16.
...
xrange = c(-1.5, 2.5)-0.5         # range of x-axis for histograms - actually to define bins
# xbigticks = -c(0.75, 1, 1.25, 1.5, 1.75, 2, 2.25)    # CHANGE TO DEPEND ON brange
xbigticks = seq(-1.5, 2.5, 0.5)
...
postscript("fitting3rep-bMinus05.eps", height = figheight, width = figwidth,
...
postscript("fitting3repMLEfix-bMinus05.eps", height = 0.8*figheight, width = 0.8*figwidth,
           horizontal=FALSE,  paper="special")  
par(omi = c(0.12, 0.05, 0.12, 0.0))      # outer margins in inches
par(mfrow=c(2,1)) #7,1))
