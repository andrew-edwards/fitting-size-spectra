# fitting3conf10000.r - as for fitting3conf.r but with xmax=10,000. 17/7/15
...
postscript("fitting3conf10000.eps", height = figheight, width = figwidth,
...
# Note for xmax=10,000: get NaN's for 753 confidence intervals - guessing that
#  with such a big range you often just get two points in the histogram,
#  so CI becomes undefined (since residuals are zero, as line fits perfectly),
#  so have to hardwire xLim here.
LT.rep.conf.sort = confPlot(LT.rep.conf, legName="(b) LT", yLab="",
    yLabels=FALSE, xLim = c(-28, 23))
     # Fixing xLim to agree with xmax=1000 figure
...
postscript("fitting3confMLEfix10000.eps", height = 0.8*figheight,
           width = 0.8*figwidth,
           horizontal=FALSE,  paper="special")  
