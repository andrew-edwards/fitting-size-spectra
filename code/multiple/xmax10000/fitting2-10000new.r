# fitting2-10000new.r - doing histogram and fit of eight methods with
#  xmax=10,000. fitting2-10000.r is based on an earlier version. 2/12/15.
...
postscript("fitting2a-10000new.eps", height = figheight, width = figwidth,
...
plot(hLBNbiom.list[["binVals"]]$log10binMid,
     ...
     ylab = "Log10 (biomass)", mgp=mgpVals, xlim=c(0, 2.9), ylim=c(2.4, 3.0),
     yaxt="n")    # Had to change axes for xmax=10000
...
axis(2, at = seq(2.4, 3, 0.05), mgp=mgpVals, tcl=-0.2, labels=rep("", 13))

lm.line(hLBNbiom.list[["binVals"]]$log10binMid, hLBNbiom.list[["unNorm.lm"]])
legJust(c("(e) LBbiom", paste("slope=",
...
plot(hLBNbiom.list[["binVals"]]$log10binMid,
     ...
     xlab=expression(paste("Log10 (bin midpoints for data ", italic(x), ")")),
     ylab = "Log10 (normalised biomass)", mgp=mgpVals, xlim=c(0,2.9),
     ylim=c(0,3), yaxt="n")
...
#  code.
postscript("standHist-10000new.eps", height = 2.7, width = figwidth/2,
...
save.image(file = "fitting2-10000new.RData")
