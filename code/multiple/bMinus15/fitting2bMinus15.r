...
n = 1000                  # sample size
b.known = -1.5              # known fixed value of b
...
if(newdata)
  ...
  {
  load(file="fitting2bMinus15.RData")    # or load in data for x
  }
...
postscript("fitting2a-bMinus15.eps", height = figheight, width = figwidth,
...
plot(hLBNbiom.list[["binVals"]]$log10binMid,
     ...
     ylab = "Log10 (biomass)", mgp=mgpVals, xlim=c(0, max(hLBNbiom.list[["binVals"]]$log10binMid)), ylim=c(2.5, max(hLBNbiom.list[["binVals"]]$log10totalBiom)), yaxt="n")
...
axis(2, at = seq(2.5, 5, 0.25), mgp=mgpVals)
axis(2, at = seq(2.5, 5, 0.05), mgp=mgpVals, tcl=-0.2, labels=rep("", 51))

lm.line(hLBNbiom.list[["binVals"]]$log10binMid, hLBNbiom.list[["unNorm.lm"]])
legJust(pos="bottomright", c("(e) LBbiom", paste("slope=",
    signif(hLBNbiom.list[["unNorm.slope"]], 1), sep=""),
...
save.image(file = "fitting2-bMinus15.RData")
