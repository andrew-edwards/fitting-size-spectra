# fitting3rep-n10000.r - redoing with n=10,000, b=-2. 25/4/16.
...
breakshist = seq(xrange[1], xrange[2], by= 4 / 81)  # = 4/3 * 61
         # bin ends at 0, 2 is centered. width of 4/81 from solving
         #  the mean of a bin equals 2, ((N+1)w + Nw)/2 = [how far true b is
         #  above min], and setting number
         #  of the bin that includes 2, N, =40 (which is how many you get with
         #  0.05, which looked good).
         #  w = **/(2N + 1) (?)     # so w can change between figures
...
postscript("fitting3rep-n10000.eps", height = figheight, width = figwidth,
...
xLim = c(-3.2, -1.5) # -0.1  # Tried shifting for fitting3rep-n10000.r to get
                             #  the low ends of LT and LTplus1, but prefer
                             #  keeping axes same as default figure, and mention
                             #  in caption.
xLimLlin = xLim + 2
ylimA = c(0, 10000)
ylimLlin = c(0, 10000)
...
print("Note that the number of estimated b values below the minimum of the
  x axis is:")
print(paste(sum(LT.rep < min(xLim)), " for LT, and ", sum(LTplus1.rep < min(xLim)), " for LTplus1; mention this in caption, so can keep the axes the same as the default one. Also the bin widths have changes slightly, but still ensure that b=-2 is in the centre of a bin. And note that y axis different from default one, because some methods have become more accurate with the larger sample size."))


# Histograms for MLE and MLEfix methods.
postscript("fitting3repMLEfix-n10000.eps", height = 0.8*figheight, width = 0.8*figwidth,
           horizontal=FALSE,  paper="special")  
par(omi = c(0.12, 0.05, 0.12, 0.0))      # outer margins in inches
par(mfrow=c(2,1)) #7,1))
...
save.image(file = "fitting3rep-n10000.RData")
