...
postscript("fitting3conf-bMinus25.eps", height = figheight, width = figwidth,
...
LT.rep.conf.sort = confPlot(LT.rep.conf, legName="(b) LT", yLab="",
    xLim = c(-25, 25), yLabels=FALSE)

LTplus1.rep.conf.sort = confPlot(LTplus1.rep.conf, legName="(c) LTplus1")

xLimCom = c(-2.65, -1.5)-0.5     # common width of axes for the remainder
...
postscript("fitting3confMLEfix-bMinus25.eps", height = 0.8*figheight,
           width = 0.8*figwidth,
           horizontal=FALSE,  paper="special")  
