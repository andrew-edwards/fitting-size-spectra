...
if(newdata)
  ...
  {
  load(file="fitting2bMinus25.RData")    # or load in data for x
  }
...
postscript("fitting2a-bMinus25.eps", height = figheight, width = figwidth,
...
plot(hLBNbiom.list[["binVals"]]$log10binMid,
     ...
     ylim=c(1.8, max(hLBNbiom.list[["binVals"]]$log10totalBiom)), yaxt="n")
...
axis(2, at = seq(1.8, 5, 0.25), mgp=mgpVals)
axis(2, at = seq(1.8, 4.3, 0.05), mgp=mgpVals, tcl=-0.2, labels=rep("", 51))

lm.line(hLBNbiom.list[["binVals"]]$log10binMid, hLBNbiom.list[["unNorm.lm"]])
legJust(pos="topright", c("(e) LBbiom", paste("slope=",
...
#  code.
postscript("standHist-bMinus25.eps", height = 2.7, width = figwidth/2,
...
save.image(file = "fitting2-bMinus25.RData")
