...
postscript("fitting2a.eps", height = figheight, width = figwidth,
...
plot( hLT.log.mids, hLT.log.counts, xlab=expression(paste("Log
 (bin midpoints for data ", italic(x), ")")), ylab = "Log (count)", mgp=mgpVals)
...
plot( hLTplus1.log10.mids, hLTplus1.log10.counts, xlab=expression(paste("Log10
 (bin midpoints for data ", italic(x), ")")), ylab = "Log10 (count+1)",
 mgp=mgpVals, yaxt="n")
...
plot(hLBNbiom.list[["binVals"]]$log10binMid,
     ...
     ylab = "Log10 (biomass)", mgp=mgpVals, xlim=c(0, 2.7), ylim=c(2.5, 3.0),
       yaxt="n")
...
# Standard histogram, but with a break in the y-axis. 
